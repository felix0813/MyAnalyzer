...
func buildRecordJSONSelect(tableExpr string) string {
	return fmt.Sprintf(`
...
func (r *Repository) Create(ctx context.Context, payload recordPayload) (Record, error) {
	...
	sql := fmt.Sprintf(`
WITH inserted AS (
    INSERT INTO browser_history (url, root_url, title, domain, visited_at, notes, visit_count)
...
%s`,
		...
		payload.VisitCount,
		buildRecordJSONSelect("inserted"),
	)
...
func (r *Repository) List(ctx context.Context, limit, offset int, search string) ([]Record, int, error) {
	...
	if trimmed := strings.TrimSpace(search); trimmed != "" {
		...
		whereClause = fmt.Sprintf("WHERE url ILIKE %s OR root_url ILIKE %s OR title ILIKE %s OR domain ILIKE %s OR notes ILIKE %s", q, q, q, q, q)
	}
...
func (r *Repository) ListRecentRootURLs(ctx context.Context, limit int) ([]RootURLSummary, error) {
	sql := fmt.Sprintf(`
SELECT COALESCE(json_agg(row_to_json(t)), '[]'::json)
FROM (
    SELECT
        root_url AS "rootUrl",
        domain,
        recent_visit_count AS "recentVisitCount",
        last_visited_at AS "lastVisitedAt",
        display_last_visited_at AS "displayLastVisitedAt",
        latest_title AS "latestTitle",
        latest_url AS "latestUrl"
    FROM v_browser_history_recent_roots
